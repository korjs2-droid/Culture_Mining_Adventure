...
function sfxFail() {
  beep(520, 0.1, 'square', 0.16);
  setTimeout(() => beep(390, 0.12, 'sawtooth', 0.17), 90);
...
function updatePlayer(dt) {
  ...
  if (!p.onGround && p.vy >= 0) {
    ...
    for (const plat of platforms) {
      if (footX < plat.x + 2 || footX > plat.x + plat.w - 2) continue;
      const surfaceY = getPlatformSurfaceY(plat, footX);
      const dist = surfaceY - bottom;
      // allow tiny penetration, and snap when close enough above surface
      if (dist >= -8 && dist <= landingTol) {
  ...
  if (!p.onGround && wasOnGround && p.vy <= 260) {
    ...
    for (const px of probes) {
      for (const plat of platforms) {
        ...
        const dist = surfaceY - bottom;
        if (dist < -8 || dist > stickTol) continue;
        const score = Math.abs(dist);
        if (!best || score < best.score) best = { y: surfaceY, score };
      }
    }
...
function drawEnemy(e) {
  ...
  const t = performance.now() * 0.004 + e.x * 0.01;
  const bob = Math.sin(t) * 2.2;
  const squash = 1 + Math.sin(t * 1.7) * 0.06;
  const cx = x + e.w * 0.5;
  const cy = e.y + e.h * 0.58 + bob;
  const rx = (e.w * 0.5) * squash;
  const ry = (e.h * 0.42) / squash;

  // Soft water glow
  ctx.save();
  ctx.globalAlpha = 0.22;
  ctx.fillStyle = '#72dbff';
  ctx.beginPath();
  ctx.ellipse(cx, e.y + e.h * 0.92, e.w * 0.58, e.h * 0.22, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.restore();

  // Main water body
  const grad = ctx.createLinearGradient(cx, e.y, cx, e.y + e.h);
  grad.addColorStop(0, '#b8f0ff');
  grad.addColorStop(0.45, '#67cbff');
  grad.addColorStop(1, '#2e97d9');
  ctx.fillStyle = grad;
  ctx.beginPath();
  ...
  ctx.beginPath();
  ctx.ellipse(cx - e.w * 0.12, cy - e.h * 0.12, e.w * 0.2, e.h * 0.1, -0.35, 0, Math.PI * 2);
  ctx.fill();

  // Bubble eyes
  ctx.fillStyle = 'rgba(255,255,255,0.95)';
  ctx.beginPath();
  ctx.arc(cx - e.w * 0.16, cy - e.h * 0.04, 4.6, 0, Math.PI * 2);
  ctx.arc(cx + e.w * 0.16, cy - e.h * 0.04, 4.6, 0, Math.PI * 2);
  ctx.fill();

  ctx.fillStyle = '#0d4f78';
  ctx.beginPath();
  ctx.arc(cx - e.w * 0.16, cy - e.h * 0.04, 2, 0, Math.PI * 2);
  ctx.arc(cx + e.w * 0.16, cy - e.h * 0.04, 2, 0, Math.PI * 2);
  ctx.fill();
}
...
function drawPlayer() {
  ...
  if (playerImageReady && spriteSheet.ready) {
    ...
    const neutralFrontPose = !state.intro && p.onGround
      && (p.onMovingBlock || (noDirInput && playerAnim.name === 'idle'));
...
bindTouchControl(touchUpBtn, pressUp, releaseUp);
